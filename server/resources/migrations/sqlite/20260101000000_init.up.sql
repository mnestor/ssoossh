...
-- carried: two tables were being rebuilt with SQLite's copy/drop/rename
-- dance just to widen a CHECK, three tables were created a few files after
-- the columns that reference them, and twenty-odd columns arrived by
-- ALTER TABLE at a distance from the table they belong to, each one
-- carrying its documentation somewhere the reader of the table never
-- looks. Collapsing them means the file you read is the schema you get.
--
...
CREATE TABLE enrollments (
    ...
    last_expired_attempt_notified_at DATETIME
);
