...
// adminConfig builds a config with the admin and auditor groups set.
func adminConfig(requireGroup, auditorGroup string) *config.Config {
	c := &config.Config{}
...
func TestAdminAuthMiddleware_Add(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			guard := NewAdminAuthMiddleware(adminConfig(tt.requireGroup, tt.auditorGroup)).Add()
...
// TestAuditorAuthMiddleware_Add covers auditor as a child role of admin:
// admin group membership satisfies auditor routes on its own, including when
// auditor_group is left unset, so narrowing the auditor group never locks
// admins out of their own audit endpoints.
func TestAuditorAuthMiddleware_Add(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			guard := NewAuditorAuthMiddleware(adminConfig(tt.requireGroup, tt.auditorGroup)).Add()
			status, reached := runAuthMiddleware(t, guard, tt.mode, tt.groups)
