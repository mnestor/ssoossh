...
func newTestSignedReplyHandler(t *testing.T, svc *CertRequestService) *SignedReplyHandler {
	...
	if err := svc.db.AutoMigrate(&model.Certificate{}, &model.User{}, &model.HostMapping{}); err != nil {
...
// TestSignedReplyHandler_ShouldCreateHostMappingForHostCertificate pins that
// host certificates with a non-empty hostname create a host_mappings row for
// the principal mapping that `host sync` will pull down.
func TestSignedReplyHandler_ShouldCreateHostMappingForHostCertificate(t *testing.T) {
	t.Parallel()
	...
	if err := h.recordCertificate(context.Background(), certmsg.SignedReply{
		...
		Serial:               50,
		KeyID:                "host-web-server",
		Principals:           []string{"web-server"},
		ValidAfter:           time.Now(),
		ValidBefore:          time.Now().Add(time.Hour),
	}); err != nil {
		t.Fatalf("unexpected error recording host certificate: %v", err)
	}

	var mapping model.HostMapping
	if err := svc.db.First(&mapping, "hostname = ?", "web-server.example.com").Error; err != nil {
		t.Fatalf("expected a host mapping row, got error: %v", err)
	}
	if mapping.Hostname != "web-server.example.com" {
		t.Errorf("got hostname %q, want %q", mapping.Hostname, "web-server.example.com")
	}
	// Principals should be JSON-encoded in the mapping
	var principals []string
	if err := json.Unmarshal([]byte(mapping.Principals), &principals); err != nil {
		t.Fatalf("failed to decode principals from host mapping: %v", err)
	}
	if len(principals) != 1 || principals[0] != "web-server" {
		t.Errorf("got principals %v, want [web-server]", principals)
	}
}

// TestSignedReplyHandler_ShouldUpdateHostMappingOnDuplicate pins that issuing
// a second host certificate for the same hostname updates the existing mapping
// rather than failing on a duplicate key constraint.
func TestSignedReplyHandler_ShouldUpdateHostMappingOnDuplicate(t *testing.T) {
	t.Parallel()

	svc := newTestCertRequestServiceWithOptions(t, config.CertificateOptions{})
	h := newTestSignedReplyHandler(t, svc)
	requestID1 := signingRequest(t, svc)
	requestID2 := signingRequest(t, svc)

	// First certificate for the hostname
	if err := h.recordCertificate(context.Background(), certmsg.SignedReply{
		RequestID:            requestID1,
		Type:                 model.CertificateTypeHost,
		Hostname:             "db-server.example.com",
		PublicKeyFingerprint: "SHA256:db1",
		Serial:               51,
		Principals:           []string{"db-primary"},
		ValidAfter:           time.Now(),
		ValidBefore:          time.Now().Add(time.Hour),
	}); err != nil {
		t.Fatalf("unexpected error recording first host certificate: %v", err)
	}

	// Second certificate for the same hostname
	if err := h.recordCertificate(context.Background(), certmsg.SignedReply{
		RequestID:            requestID2,
		Type:                 model.CertificateTypeHost,
		Hostname:             "db-server.example.com",
		PublicKeyFingerprint: "SHA256:db2",
		Serial:               52,
		Principals:           []string{"db-primary", "db-replica"},
		ValidAfter:           time.Now(),
		ValidBefore:          time.Now().Add(2 * time.Hour),
	}); err != nil {
		t.Fatalf("unexpected error recording second host certificate: %v", err)
	}

	// Should have exactly one mapping row (not duplicated)
	var count int64
	if err := svc.db.Model(&model.HostMapping{}).
		Where("hostname = ?", "db-server.example.com").
		Count(&count).Error; err != nil {
		t.Fatalf("unexpected error counting host mappings: %v", err)
	}
	if count != 1 {
		t.Errorf("expected 1 host mapping row for the hostname, got %d", count)
	}

	// The mapping should have the updated principals from the second certificate
	var mapping model.HostMapping
	if err := svc.db.First(&mapping, "hostname = ?", "db-server.example.com").Error; err != nil {
		t.Fatalf("unexpected error reading the mapping: %v", err)
	}
	var principals []string
	if err := json.Unmarshal([]byte(mapping.Principals), &principals); err != nil {
		t.Fatalf("failed to decode principals: %v", err)
	}
	if len(principals) != 2 || principals[0] != "db-primary" || principals[1] != "db-replica" {
		t.Errorf("got principals %v, want [db-primary db-replica]", principals)
	}
}

// TestSignedReplyHandler_ShouldNotCreateHostMappingForUserCertificate pins
// that user certificates (which don't have a hostname) don't create a host
// mapping row.
func TestSignedReplyHandler_ShouldNotCreateHostMappingForUserCertificate(t *testing.T) {
	t.Parallel()
	...
	if err := h.recordCertificate(context.Background(), certmsg.SignedReply{
		RequestID:            requestID,
		Type:                 model.CertificateTypeUser,
		PublicKeyFingerprint: "SHA256:user",
		Serial:               53,
		KeyID:                "alice",
		Principals:           []string{"alice"},
		ValidAfter:           time.Now(),
		...
		t.Fatalf("unexpected error recording user certificate: %v", err)
	}

	var count int64
	if err := svc.db.Model(&model.HostMapping{}).Count(&count).Error; err != nil {
		t.Fatalf("unexpected error counting host mappings: %v", err)
	}
	if count != 0 {
		t.Errorf("expected 0 host mapping rows for a user certificate, got %d", count)
	}
}

// TestSignedReplyHandler_ShouldNotCreateHostMappingWithEmptyHostname pins
// that host certificates with an empty hostname (which shouldn't happen but
// must be guarded against) don't create a mapping row.
func TestSignedReplyHandler_ShouldNotCreateHostMappingWithEmptyHostname(t *testing.T) {
	t.Parallel()
	...
	if err := h.recordCertificate(context.Background(), certmsg.SignedReply{
		...
		t.Fatalf("unexpected error recording host certificate with empty hostname: %v", err)
	}
	...
	if count != 0 {
		t.Errorf("expected 0 host mapping rows for a host cert with empty hostname, got %d", count)
	}
}
