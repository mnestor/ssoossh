...
// (required for multi-instance deployments). See
// https://mnestor.github.io/ssoossh/internals/architecture/ for how the
// certificate pipeline uses this.
package pubsub
...
// consumers (the signer and listener/resolver in
...
func newGoChannel(wmLogger watermill.LoggerAdapter) (*PubSub, error) {
	channel := gochannel.NewGoChannel(gochannel.Config{
		...
		//     (https://mnestor.github.io/ssoossh/internals/architecture/): gochannel replays a
		...
		//     https://mnestor.github.io/ssoossh/internals/architecture/'s now-resolved caveat
		//     about this).
		//
...
// has no replay, so anything published in that gap is dropped outright -- and
// for a certificate request a dropped wake is unrecoverable, not merely late:
// the certificate is deliberately never persisted (see
// https://mnestor.github.io/ssoossh/internals/architecture/), so the instance
// that missed it can read "approved" from the database and still have nothing
// to hand its client, which then blocks until the request expires.
//
