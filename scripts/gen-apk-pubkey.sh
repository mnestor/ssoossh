#!/usr/bin/env sh
# Extracts the public half of the apk signing key into build/, from where
# the release picks it up as a published artifact.
#
...
#
# Writes to build/ for the same reason the keyring generator does:
# goreleaser asserts dist/ is empty after the before hooks have run, so a
# hook cannot stage anything there.
#
