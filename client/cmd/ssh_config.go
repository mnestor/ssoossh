...
const sshConfigGuidance = `The client is never run on its own — ssh invokes it. Two ways to arrange that:
  ...
  The IdentityFile line matters only when use_agent is false, and is
  harmless with an agent. Without it an agentless host offers no key at all:
  the client writes to whatever key_filename resolves to -- ~/.ssh/id_ssoossh
  by default -- and that is not a name ssh tries on its own. No
  CertificateFile line is needed; ssh derives id_ssoossh-cert.pub from
  IdentityFile's own name.
