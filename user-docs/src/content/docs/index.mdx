...
hero:
  ...
  actions:
    ...
    - text: How it works
      ...
      variant: minimal
  image:
    # Raw HTML rather than `file`: Starlight renders a hero file at 400x400,
    # which crops a wide screenshot to its middle. The file lives in public/
    # so it is served as-is; src/styles/ssoossh.css frames it.
    html: '<img src="/ssoossh/screens/dashboard-hero.png" alt="The ssoossh web UI, showing the navigation rail and a list of recently approved certificates" width="1372" height="760" loading="eager" decoding="async" />'
---
...
```mermaid
sequenceDiagram
    accTitle: The shape of a certificate login
    accDescr {
      The user runs ssh to a host. Through Match exec or ProxyCommand, ssh
      invokes the ssoossh client, which sends its public key and requested
      options to ssoosshd and opens a browser. The user signs in at the
      identity provider, which returns their identity and groups, and
      approves. ssoosshd returns a signed certificate; the client loads it
      into ssh-agent; ssh connects to the target host, which verifies the
      certificate against TrustedUserCAKeys.
    }
    autonumber
    actor U as You
