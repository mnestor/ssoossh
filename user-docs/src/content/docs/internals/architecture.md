...
for anyone reading or changing the server's code. It consolidates the six
implementation phase documents that built the pipeline; those are gone, but
every decision that still constrains the code is recorded here.
...
[`pubsub.backend`](/ssoossh/reference/config/pubsub/#backend) set to `nats`,
the same pipeline runs across `ssoosshd serve api` and `ssoosshd sign`
processes -- see [startup modes](/ssoossh/operations/startup-modes/).
...
| `certrequest.sign` | shared queue | none -- at-most-once by decision, the human retries (see [Decisions](/ssoossh/project/decisions/)) |
...
Steps 8 and 9 look like the api process talking to itself, and in this
...
1. Client `POST`s a request, and gets `events_url` + `approval_url`.
2. Client opens `events_url` (SSE), so `Wait` subscribes to
   `certrequest.wait.<id>`.
...
5. The listener writes the audit row, delivers the certificate, marks the
   request terminal.
6. The client's `Wait` returns, and `eventsHandler` sends the terminal SSE
   event.
...
sequenceDiagram
    ...
    actor Approver as browser
    participant Signer as signer
    participant Listener as listener

    Client->>API: POST request
    API-->>Client: events_url + approval_url
    Client->>API: open SSE, Wait subscribes
    Approver->>API: authenticate and approve
    API->>API: resolve policy, row = signing
    API-)Signer: job [certrequest.sign]
    Signer->>Signer: sign with CA key
    Signer-)Listener: reply [certrequest.signed]
    Listener->>Listener: audit row, then cache + wake
    Listener->>Listener: mark request terminal
    Listener-)API: wake [certrequest.wait.id]
    API-->>Client: certificate over SSE
```

See [Interactive user certificates](/ssoossh/concepts/user-certificate/) for
the user-facing sequence diagrams of the same path.
...
(certificate lifetime policy, LDAP to principals), so it implements the
...
`SigningGrace` derive from
[`cert_options.client_timeout`](/ssoossh/reference/config/cert_options/#client_timeout),
which must be positive, so there is no configuration in which the formula
degenerates and no special case for an unbounded approval window. The sweep
also runs on the `SigningGrace` interval, which is why the worst-case client
wait spends two of those shares and why `ApprovalTTL` reserves for both -- the
three add up to exactly `client_timeout`.
...
- The sweep's age-threshold approach is safe for several instances sharing a
  ...
  budget. See
  [multi-instance-safety-plan.md](https://github.com/mnestor/ssoossh/blob/main/docs/dev/multi-instance-safety-plan.md).
