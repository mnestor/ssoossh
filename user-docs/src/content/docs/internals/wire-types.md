...
forces them to agree, so each pairing has an explicit mechanism. This page is
for anyone changing a wire struct or adding an endpoint.
...
- `frontend/src/lib/api/types.ts` is the only hand-written file left, and it
  ...
  uses (`RequestDetailResponse` becomes `RequestDetail`).
...
changes nothing and runs in CI. The rendered result is the
