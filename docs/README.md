...
Designs for work that has not been built. Everything in
[proposals/](proposals/) is a proposal until the code lands; a design record
for something already shipped belongs beside the feature it describes, or in
[dev/](dev/). Each one states its status and the commit its `file:line`
anchors were verified against, because those anchors drift.
...
| [config-coordination.md](proposals/config-coordination.md) | Detecting and reporting configuration divergence between instances in a multi-instance deployment |
| [service-retrieval-anomaly-policy.md](proposals/service-retrieval-anomaly-policy.md) | Alerting and locking a service enrollment code redeemed from too many source networks |
| [source-address-restrictions.md](proposals/source-address-restrictions.md) | Approver-chosen source-address pinning and a retrieval allowlist for service enrollments |
