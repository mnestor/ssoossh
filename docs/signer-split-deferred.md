...
isolated from the webserver for CA key hygiene. The signer stays in-process
for now, with the split *seam* preserved instead: `server/certmsg`,
`server/signer`'s zero-database test, and `server/bootstrap/pipeline.go`.
