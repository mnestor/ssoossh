...
Resolved with a default at implementation time, revisit if wrong: tier 2
gates from day one, the harness IdP stayed in `test/e2e/harness`, and tier-3
`sshd` runs in the same PR-gating job as tiers 1–2.
