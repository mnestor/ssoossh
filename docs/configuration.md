...
the settings that matter and links to the annotated sample with every
remaining default:

- [ssoosshd.yaml.default](ssoosshd.yaml.default): full server reference
- [ssoossh.yaml.default](ssoossh.yaml.default): full client reference
...
semantics and [ssoosshd.yaml.default](ssoosshd.yaml.default) for the
syntax.
...
Verify it resolves before a real login:
...
[ssoossh.yaml.default](ssoossh.yaml.default). Two worth knowing:
