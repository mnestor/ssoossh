...
Revocation exists as well, but a revocation list has to be distributed to every host by
a separate scheduled task — and it stops mattering much once certificates are
short-lived enough to expire before a revocation would reach anyone. ssoossh does not
publish one at all, deliberately: user and service certificates are short-lived enough
that a list could not outrun them, and host certificates — the only long-lived ones —
would need every *client* to consult it, which clients do not do. Revocation therefore
happens where issuance happens: disable the account, and the credentials that mint
certificates stop working. See release-plan.md's Decisions table, the
Revocation row, for the current statement of this position. What is usually
...
> **Open design question.** The current shape is essentially "the server vouched for
> this user just now." Whether a PAM module generating and discarding key material is
> the right primitive at all is unresolved, and is being worked through in a separate
> design document. Two commonly suggested answers do not apply here:
> `AuthorizedPrincipalsCommand` is an `sshd_config` directive with no bearing on PAM,
> and `pam_ssh_agent_auth` depends on ssh agent forwarding, which is not an acceptable
> requirement.
...
certificate through a key revocation list.



- SSH certificate format — [draft-miller-ssh-cert-01](https://www.ietf.org/archive/id/draft-miller-ssh-cert-01.html)
