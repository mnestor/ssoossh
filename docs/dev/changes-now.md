...
| 5 | Medium | Run a fresh `pnpm audit` and stop relying on the checked-in `pnpm-audit-results.json` snapshot for release sign-off. Process item, not code. | **Verified**: `pnpm-audit-results.json` does not exist in the repository (not tracked, not untracked). Fresh `pnpm audit --audit-level high` in `frontend/` shows "No known vulnerabilities found" as of 2026-08-22. |
