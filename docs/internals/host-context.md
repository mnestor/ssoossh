# Host context: what a PAM or console request says about where it came from
...
This document is the reference for those fields: what each one is, where
...
See `server/service/audit.go` for the taxonomy and
[signing-pipeline.md](signing-pipeline.md) for where each stage sits.
